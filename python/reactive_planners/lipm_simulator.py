...
class LipmSimpulator:
    ...
    def step(self, time, u_current_step, previous_x_com, previous_xd_com, contact):
        if contact is 0:
            return self.floating_base(time, previous_x_com, previous_xd_com)
        else:
            return self.LIPM(time, u_current_step, previous_x_com, previous_xd_com)

    def floating_base(self, time, previous_x_com, previous_xd_com):
        ...
        self.x_com[:2] = self.xd_com[:2] * (time - self.t_s + self.control_loop) + self.last_x_com[:2]
        self.x_com[2] = -0.5 * 9.81 * (time - self.t_s + self.control_loop) * (time - self.t_s + self.control_loop) + \
                        self.last_xd_com[2] * (time - self.t_s + self.control_loop) + self.last_x_com[2]
        self.xd_com[2] = -9.81 * (time - self.t_s + self.control_loop) + self.last_xd_com[2]
        self.xdd_com[:] = [[0], [0], [0]]
