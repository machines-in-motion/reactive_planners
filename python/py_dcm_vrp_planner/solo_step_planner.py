...
class SoloStepPlanner:
    ...
    def compute_next_step_locations(self, q, dq, t, u_t, u_t_min, u_t_max, t_end, cnt_array, v_des, W):
        """
        ...
        FL_hip, FR_hip, HL_hip, HR_hip = self.compute_kinematic_constraints(q,dq,t,t_end,v_des, self.ht)
                
        ## Creating Variables
        fl_x = m.addVar(lb= FL_hip[0] + self.kin_min[0] , ub= FL_hip[0] + self.kin_max[0], name = "fl_x")
        fl_y = m.addVar(lb= FL_hip[1] + self.kin_min[1] , ub= FL_hip[1] + self.kin_max[1], name = "fl_y")
        
        fr_x = m.addVar(lb= FR_hip[0] + self.kin_min[0] , ub= FR_hip[0] + self.kin_max[0], name = "fr_x")
        fr_y = m.addVar(lb= FR_hip[1] + self.kin_min[1] , ub= FR_hip[1] + self.kin_max[1], name = "fr_y")
        
        hl_x = m.addVar(lb= HL_hip[0] + self.kin_min[0] , ub= HL_hip[0] + self.kin_max[0], name = "hl_x")
        hl_y = m.addVar(lb= HL_hip[1] + self.kin_min[1] , ub= HL_hip[1] + self.kin_max[1], name = "hl_y")
        
        hr_x = m.addVar(lb= HR_hip[0] + self.kin_min[0] , ub= HR_hip[0] + self.kin_max[0], name = "hr_x")
        hr_y = m.addVar(lb= HR_hip[1] + self.kin_min[1] , ub= HR_hip[1] + self.kin_max[1], name = "hr_y")
        ...
        # creating cost
        c1 = W[0] *sl_1 * sl_1 + W[1] *sl_2 * sl_2 + W[2] *sl_3 * sl_3 + W[3] *sl_4 * sl_4 \
            + W[4]* sl_5*sl_5 + W[5]* sl_6*sl_6 + W[6]* sl_7*sl_7 + W[7]* sl_8*sl_8 + W[8]* sl_9*sl_9 \
             +   W[9]* sl_10*sl_10
        ...
        m.addConstr(cnt_array[0]*fl_x + cnt_array[1]*fr_x + cnt_array[2]*hl_x + cnt_array[3]*hr_x == np.sum(cnt_array) * (u_t[0]) + sl_1, "sl_constr_1")
        m.addConstr( cnt_array[0]*fl_y + cnt_array[1]*fr_y + cnt_array[2]*hl_y + cnt_array[3]*hr_y == np.sum(cnt_array) * (u_t[1]) + sl_2, "sl_constr_2")

        
        m.optimize()
        x_opt = m.getVars()
        
        return x_opt[0].x, x_opt[1].x, x_opt[2].x, x_opt[3].x, x_opt[4].x, x_opt[5].x, x_opt[6].x, x_opt[7].x
