...
PYBIND11_MODULE(reactive_planners, m) {
  ...
  pybind11::class_<DcmVrpPlanner>(m,"DcmVrpPlanner")
    .def(pybind11::init<>())
    ...
    .def_property_readonly("get_world_M_local", &DcmVrpPlanner::get_world_M_local)
    .def_property_readonly("get_dcm_local", &DcmVrpPlanner::get_dcm_local)
    .def_property_readonly("get_current_step_location_local", &DcmVrpPlanner::get_current_step_location_local)
    ...
    .def_property_readonly("get_next_step_location", &DcmVrpPlanner::get_next_step_location)
    .def_property_readonly("get_duration_before_step_landing", &DcmVrpPlanner::get_duration_before_step_landing)

    // String representation of the sovler.
    .def("__repr__", [](const DcmVrpPlanner &planner) { return planner.to_string(); } );
