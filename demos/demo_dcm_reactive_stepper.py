...
if __name__ == "__main__":
    ...
    for i in range(N):
        ...
        dcm_reactive_stepper.run(
            time, dcm_reactive_stepper.get_left_foot_position(),
    ...
    plt.plot(plt_time, np.array(plt_time_from_last_step_touchdown), label="time_from_last_step_touchdown")
    plt.plot(plt_time, np.array(plt_step_duration), label="step_duration")
    plt.legend()

    plt.figure("support_foot")
    plt.plot(plt_time, np.array(plt_current_support_foot)[:, 0], label="current support x")
    plt.plot(plt_time, np.array(plt_current_support_foot)[:, 1], label="current support y")
    plt.plot(plt_time, np.array(plt_current_support_foot)[:, 2], label="current support z")
    plt.legend()
